...
Bitmap::Bitmap(uint64_t* bitmap, size_t bytes, bool zero, bool max)
...
void Bitmap::Clear(size_t base, size_t length)
{
    _lock.Acquire();
    if(base > Max() || (base + length - 1) > Max()) 
        FATAL("Overflow detected on Bitmap while clearing bits.");
...
uint64_t Bitmap::FindFirstClear()
{
    _lock.Acquire();
...
uint64_t Bitmap::FindFirstClear(size_t length)
{
    _lock.Acquire();
...
uint64_t Bitmap::FindLastClear(size_t length)
{
    _lock.Acquire();
    size_t frames = length / BITS_PER_FRAME;
    if((length % BITS_PER_FRAME) != 0) frames++;
...
uint64_t Bitmap::FindLastClear(size_t length, size_t alignment)
{
    _lock.Acquire();
    size_t frames = length / BITS_PER_FRAME;
    if((length % BITS_PER_FRAME) != 0) frames++;
...
void Bitmap::Set(size_t base, size_t length)
{
    _lock.Acquire();
    if(base > Max() || (base + length - 1) > Max())         
    {
        ...
        FATAL("Overflow detected on Bitmap while setting bits.");
    }
    size_t startQuad = base / BITS_PER_FRAME;
    size_t endQuad = (base + length - 1) / BITS_PER_FRAME;
