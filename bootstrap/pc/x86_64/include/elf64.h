    ...
    private:
        ...
        inline SectionHeader& GetSectionHeader(const char* name)
        {
            for(int i = 0; i < _header->e_shnum; i++)
            {
                ...
                if(String::length((const char*)sectionName) == 0) continue;
                if(String::compare(name, (const char*)sectionName) == 0) return sectionHeader;
            }
        ...
        inline Symbol& GetSymbol( const char* name )
        {
            ...
            for(size_t i = 0; i < GetSymbolTableEntryCount(); i++)
            {
                ...
                if(String::compare(name, symbolName) == 0) return *symbol;
            }
