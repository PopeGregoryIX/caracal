...
class Bitmap
{
    ...
    public:
        Bitmap() : _bitmap(nullptr), _bytes(0){}

        Bitmap(uint64_t* bitmap, size_t bytes, bool zero, bool max);

        inline uint64_t* GetBitmap( void ) { return _bitmap; }
        ...
        void operator=(Bitmap bmp) { _bitmap = bmp._bitmap; _bytes = bmp._bytes; }
};
